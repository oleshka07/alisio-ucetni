...
echo "== clients";      q 'select c.type, (p.ico is not null) as ico, (p.dic is not null) as dic, (p."dataBox" is not null) as databox from "Client" c left join "CompanyProfile" p on p."clientId" = c.id'
echo "== users";        q 'select role, ("telegramChatId" is not null) as telegram, "isActive" from "User"'
echo "== bank accounts (перед sync)"; q 'select name, source, "isActive", "lastSyncAt", left("lastError",160) from "BankAccount"'
echo "== inboxes (перед sync)";       q 'select name, kind, "isActive", "lastSyncAt", left("lastError",160) from "DocumentInbox"'
echo "== run sync"; bash deploy/cron.sh sync 2>&1 | mask | tail -3; echo "exit ${PIPESTATUS[0]}"
echo "== bank accounts (після)"; q 'select name, "lastSyncAt", left("lastError",160) from "BankAccount"'
echo "== inboxes (після)";       q 'select name, "lastSyncAt", left("lastError",160) from "DocumentInbox"'
